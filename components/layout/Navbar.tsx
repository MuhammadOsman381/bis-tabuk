...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-black/5 bg-white/88 shadow-sm shadow-[#1A1F4B]/5 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex justify-between items-center">
        ...
        <Link href="/" className="flex items-center gap-3 rounded-2xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#C8102E]/15">
          ...
          <div className="min-w-0">
            <strong className="text-[#1A1F4B] text-base sm:text-lg leading-tight block">British International</strong>
        ...
        <div className="hidden lg:flex items-center gap-1.5">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className="px-3.5 py-2 text-sm font-semibold text-[#1A1F4B]/80 hover:text-[#C8102E] rounded-full hover:bg-[#C8102E]/7 transition-all"
            >
              {item.label}
            </Link>
          ))}
          ...
          <Link 
            href="#admissions" 
            className="ml-3 bg-[#C8102E] text-white px-5 py-3 rounded-full font-bold text-sm shadow-lg shadow-[#C8102E]/20 hover:bg-[#9B0D23] hover:-translate-y-0.5 transition-all"
          >
            Apply Now
          </Link>
        </div>
            ...
            <div className="px-4 py-4 flex flex-col gap-2 text-base">
              {navItems.filter((item, index, array) => array.findIndex((candidate) => candidate.label === item.label) === index).map((item) => (
                <Link
                  ...
                  className="rounded-2xl px-4 py-3 font-semibold text-[#1A1F4B] hover:bg-[#FFF8F0] hover:text-[#C8102E] transition"
                >
                  {item.label}
                </Link>
              ))}
              <Link href="#admissions" onClick={() => setIsOpen(false)} className="mt-2 bg-[#C8102E] text-white py-3 rounded-full text-center font-bold shadow-lg shadow-[#C8102E]/20">Apply Now</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
