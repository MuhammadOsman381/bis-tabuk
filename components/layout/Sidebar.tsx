...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col">
      {/* <div className="border-b border-zinc-200/70 px-5 py-5 dark:border-white/10">
        <Link href="#" onClick={onNavigate} className="group flex items-center gap-3 rounded-2xl">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#C8102E] text-sm font-black text-white shadow-lg shadow-[#C8102E]/25 dark:shadow-[#C8102E]/35">
            B
          </div>
          <div>
            <p className="text-sm font-bold leading-tight text-zinc-950 dark:text-zinc-50">British International</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#C8102E]">School of Tabuk</p>
          </div>
        </Link>
      </div> */}
      ...
      <div className="border-t border-zinc-200/70 p-4 dark:border-white/10">
        <Link
          href="/apply"
          onClick={onNavigate}
          className="group relative isolate flex items-center justify-center overflow-hidden rounded-2xl bg-[linear-gradient(135deg,#E11D48,#C8102E_45%,#7F1024)] px-4 py-3 text-sm font-bold text-white shadow-lg shadow-[#C8102E]/20 transition hover:-translate-y-0.5 dark:shadow-[#C8102E]/30"
        >
...
export default function Sidebar({ isDesktopOpen, showMobileTrigger = false }: SidebarProps) {
  ...
  return (
    <>
      <motion.aside
        ...
        className="fixed inset-y-0 left-0 z-[60] hidden w-80 border-r border-zinc-200/80 bg-white/86 shadow-2xl shadow-zinc-900/10 backdrop-blur-2xl dark:border-white/10 dark:bg-zinc-950/88 dark:shadow-black/40 lg:block"
      >
        {/* <button
          type="button"
          onClick={onDesktopClose}
          aria-label="Close sidebar navigation"
          className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-zinc-200 bg-white text-zinc-700 shadow-sm transition hover:bg-zinc-100 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-white/10"
        >
          <X className="h-4 w-4" />
        </button> */}
        <SidebarContent />
      ...
      {showMobileTrigger && (
        <button
          ...
          className="fixed bottom-5 left-5 z-[70] inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-[#C8102E] text-white shadow-2xl shadow-[#C8102E]/25 transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#C8102E]/20 dark:shadow-[#C8102E]/35 lg:hidden"
        >
        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              exit={{ x: '-100%' }}
              transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}
              className="fixed inset-y-0 left-0 z-[90] w-[min(88vw,22rem)] border-r border-zinc-200/80 bg-white/92 shadow-2xl shadow-black/20 backdrop-blur-2xl dark:border-white/10 dark:bg-zinc-950/94 lg:hidden"
            >
              <button
                type="button"
                onClick={() => setIsOpen(false)}
