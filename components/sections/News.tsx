...
export default function News() {
  return (
    <section
      id="news"
      className="relative overflow-hidden bg-[#FFF8F0] py-20 sm:py-24"
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#C8102E]/20 to-transparent" />
      <div className="absolute right-0 top-16 h-72 w-72 rounded-full bg-[#C9A84C]/10 blur-3xl" />
      <div className="absolute bottom-10 left-0 h-80 w-80 rounded-full bg-[#C8102E]/8 blur-3xl" />

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <Reveal className="mb-12 grid gap-8 lg:mb-14 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#C8102E]/10 bg-white px-4 py-2 shadow-sm shadow-[#1A1F4B]/5">
              <Sparkles className="w-4 h-4 text-[#C8102E]" />
              <span className="text-[#C8102E] font-semibold text-xs tracking-[2px] uppercase">
                Latest News
              </span>
            </div>

            <h2 className="text-4xl md:text-5xl font-bold mt-5 text-[#1A1F4B] leading-tight">
              Together We Are{' '}
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center lg:flex-col lg:items-end">
            <div className="grid grid-cols-3 overflow-hidden rounded-2xl border border-black/5 bg-white/80 shadow-lg shadow-[#1A1F4B]/5 backdrop-blur">
              {stats.map((stat) => (
                <div key={stat.label} className="border-r border-gray-100 px-4 py-3 text-center last:border-r-0">
                  <div className="text-xl font-bold text-[#1A1F4B]">{stat.value}</div>
                  <div className="mt-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-gray-400">{stat.label}</div>
                </div>
              ))}
            </div>
            <a
              href="#"
              className="group inline-flex items-center justify-center gap-2 rounded-full bg-[#C8102E] px-5 py-3 text-sm font-bold text-white shadow-lg shadow-[#C8102E]/20 transition-all hover:-translate-y-0.5 hover:bg-[#9B0D23]"
            >
              All News & Updates
        ...
        >
          ...
          <motion.article
            ...
            whileHover={{ y: -6 }}
            className="group relative overflow-hidden rounded-2xl border border-black/5 bg-white shadow-[0_18px_50px_rgba(26,31,75,0.08)] transition-shadow duration-500 hover:shadow-[0_24px_70px_rgba(26,31,75,0.12)] lg:col-span-3"
          >
            <div className="relative h-[320px] overflow-hidden sm:h-[420px]">
              ...
              <div className="absolute left-5 top-5 flex flex-wrap gap-3 sm:left-6 sm:top-6">
                <div className="flex items-center gap-2 bg-white/90 backdrop-blur-md text-[#1A1F4B] text-xs font-semibold px-4 py-2 rounded-full">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-8 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-5 text-sm text-gray-500">
                  ...
                  <div className="hidden items-center gap-2 rounded-full bg-gray-50 px-3 py-2 sm:flex">
                ...
                <a
                  href="#"
                  className="group inline-flex items-center gap-2 font-semibold text-[#C8102E]"
                >
                  View Post
                  <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                </a>
            ...
            {sideNews.map((item, index) => {
              ...
              return (
                <motion.article
                  ...
                  variants={{ hidden: { opacity: 0, y: 24 }, visible: { opacity: 1, y: 0 } }}
                  whileHover={{ y: -5 }}
                  className="group flex flex-col gap-4 rounded-2xl border border-black/5 bg-white p-4 shadow-[0_12px_34px_rgba(26,31,75,0.06)] transition-shadow duration-300 hover:shadow-[0_18px_50px_rgba(26,31,75,0.1)] sm:flex-row sm:gap-5"
                >
                  {/* Image */}
                  ...
                  <div className="flex flex-col  justify-between flex-1 min-w-0 py-1">
                    <div className="space-x-2" >
                      ...
                      <p className="mt-4 inline-flex items-center gap-1.5 text-xs font-medium text-gray-400">
                        <CalendarDays className="h-3.5 w-3.5" />
                        {item.date}
                      </p>

                      <h4 className="mt-2 text-[17px] font-bold leading-snug text-[#1A1F4B] transition-colors group-hover:text-[#C8102E] line-clamp-3">
                        {item.title}
                      </h4>
                    </div>
                    ...
                    <a
                      href="#"
                      className="inline-flex items-center gap-2 mt-5 text-sm font-semibold text-[#C8102E]"
                    >
                      Read More
                      <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                    </a>
